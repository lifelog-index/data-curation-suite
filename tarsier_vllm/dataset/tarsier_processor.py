...
from transformers import Qwen2VLImageProcessor, Qwen2VLImageProcessorFast
...
class TarsierProcessor(ProcessorMixin):
    ...
    def get_num_vision_tokens(self, content):
        if isinstance(self.image_processor, Qwen2VLImageProcessor) or isinstance(self.image_processor, Qwen2VLImageProcessorFast):
            merge_length = self.image_processor.merge_size**2
            if content['type'] == 'image':
                num_image_tokens = content['image']['image_grid_thw'].prod() // merge_length
            else:
                # video
                num_image_tokens = content['video']['image_grid_thw'].prod() // merge_length
            return num_image_tokens
        else:
            # Nhánh else xử lý cách tính số image tokens cho các mô hình vision khác
