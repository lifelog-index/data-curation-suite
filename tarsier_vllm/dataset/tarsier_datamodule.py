...
from tarsier_vllm.dataset.utils import format_one_sample

class InferenceDataset(Dataset):
    def __init__(self, samples: List[Dict]=None, config: Dict=None, processor: Union[TarsierDataProcessor, TarsierProcessor, str]=None):
        self.config = DictToObject(config) if isinstance(config, dict) else config
        if not isinstance(processor, TarsierDataProcessor):
            self.processor = init_processor(processor, config)
        else:
    ...
    def add_sample(self, media_file=None, prompt="Describe the video in detail."):
        sample = format_one_sample(media_file=media_file, prompt=prompt)
    ...
    def __getitem__(self, index):
        if index < 0 or index >= len(self.samples):
            raise IndexError("Index out of range")
        try:
            input_data = self.samples[index]
            import pdb; pdb.set_trace()
            input_ids = self.processor(input_data)
        except Exception as e:
            print(f"Load data error: {e}")
            return input_data, None
        return input_data, input_ids